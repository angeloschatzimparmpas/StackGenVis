...
export default function loom() {
  ...
  let widthInner = () => 30;
  let value = d => d.value;
  let outercolorfun = d => d.outercolorfun;
  let inner = d => d.inner;
  let outer = d => d.outer;
  ...
  function loomLayout(layoutData) {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      for (j = 0; j < s; j += 1) {
        ...
        outercolor = outercolorfun(data[di].values[dj]);
        const a0 = x;
        x += v * k;
        const a1 = x;
        subgroups[counter] = {
  ...
  loomLayout.outercolorfun = function(_) {
    return arguments.length ? ((outercolorfun = _), loomLayout) : outercolorfun;
  };
