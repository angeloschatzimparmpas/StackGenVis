...
function(d3,d3tip)
{
  ...
  var brushList = []
  var default_colors = ['#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c','#fdbf6f','#ff7f00','#cab2d6','#6a3d9a','#b15928']
  ...
  var exploding_boxplot = function(data,aes)
  {
    ...
    var chart = function(elem)
    {
      ...
      var yaxisEl = container.append('g')
		...
		function brushmove() {
		  while (brushList.length > 0){
			brushList.pop()
		  }
		  brushList.push(brush.extent()[1])
		  brushList.push(brush.extent()[0])
		  if (brushList[0] == brushList[1]) {
			implode_boxplot()
		  } else {
			explode_boxplot_all()
		  }
		...
		var create_jitter = function(g,i) {
			...
			d3.select(this).append('g')
					...
					.on('click',function(d){
						implode_boxplot_each(this.parentNode,g)
					})
		...
		var init_jitter = function(s){
			s.attr('class',function(d){
						return 'd3-exploding-boxplot point '+d.Algorithm
				})
		...
		var explode_boxplot = function(elem,g){
			...
			d3.select(elem).selectAll('.normal-points')
 					...
 					.enter()
						...
						.duration(function(){
							return 100+300*Math.random()
						})
						.call(draw_jitter)
		};
		var explode_boxplot_all = function(){
		...
		var implode_boxplot_each = function(elem,g){
			d3.select(elem).selectAll('.normal-points')
						.each(function(g){
							d3.select(this)
								.selectAll('circle')
			...
			d3.select(elem).selectAll('g.box')
					.transition()
					.ease(d3.ease('back-out'))
					.duration(100)
					.delay(200)
